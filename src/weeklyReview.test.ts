import { weeklyReview } from './tools';
  ...
  it('should return a weekly review object', async () => {
    ...
    const mockApi = {
      getTasks: jest.fn().mockImplementation((params) => {
        if (params.projectId === 'inboxId') return Promise.resolve({ results: inboxTasks });
        if (params.filter === 'overdue') return Promise.resolve({ results: overdueTasks });
        if (params.label === 'waiting_for_💤') return Promise.resolve({ results: waitingForTasks });
        if (params.projectId === 'somedayMaybeId') return Promise.resolve({ results: somedayMaybeTasks });
        return Promise.resolve({ results: [] });
      }),
      getProjects: jest.fn().mockResolvedValue({ results: projects }),
    };

    const reviewData = await weeklyReview(mockApi as any, 'inboxId', 'somedayMaybeId');
    ...
    expect(reviewData).toEqual({
      ...
      somedayMaybeTasks: [{ id: '5', content: 'Someday/Maybe Task', children: [] }],
    });
  });
});
