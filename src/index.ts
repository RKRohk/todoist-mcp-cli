...
export async function main() {
    ...
    switch (command) {
        case 'add':
            const taskContent = args.slice(1).filter(arg => !arg.startsWith('@')).join(' ')
            const labels = args.slice(1).filter(arg => arg.startsWith('@')).map(label => label.substring(1))
            if (!taskContent) {
            ...
            break
        case 'list':
            const listLabel = args[1] && !args[1].startsWith('--') ? args[1] : undefined
            const listProjectId = args.find(arg => arg.startsWith('--projectId='))?.split('=')[1]
        ...
        case 'update':
            ...
            const updateOptions: { content?: string; dueString?: string; priority?: number; labels?: string[] } = {}
